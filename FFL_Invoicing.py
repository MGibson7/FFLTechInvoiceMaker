...
prefs = {"profile.default_content_settings.popups": 0,
             "download.default_directory":
                        r"C:\Users\thegi\PycharmProjects\viasat_invoicing\\",
             "directory_upgrade": True}
options.add_experimental_option("prefs", prefs)

driver = webdriver.Chrome(executable_path=r'C:\Users\thegi\PycharmProjects\viasat_invoicing\chromedriver.exe', options = options)




#tech first name here, invoice start and end dates, invoice name and date (as it will appear on invoice)
invoice_name = ""
...
try:
    elem = driver.find_element_by_id("browseOrder:orderGrid:filterBoard:createdDateFilter:j_idt172") or driver.find_element_by_id("browseOrder:orderGrid:filterBoard:createdDateFilter:j_idt196")
    elem.click()
...
path = r'C:\Users\thegi\PycharmProjects\viasat_invoicing\\'
...
def Make_Invoice(tech):
    ...
    for num in range(c):
        ...
        if a == tech:
            ...
            job_names.append(df.iloc[x, 15])
            order_type.append(df.iloc[x, 8])
            order_region.append(df.iloc[x, 30])
            if (df.iloc[x, 8]) == "Install":
                if (df.iloc[x, 30]) == "NC09":
                    job_pay.append(150)
                elif (df.iloc[x, 30]) == "NC08":
                    job_pay.append(150)
                elif (df.iloc[x, 30]) == "NC36":
                    job_pay.append(150)
                elif (df.iloc[x, 30]) == "NC03":
                    job_pay.append(150)
                else:
                    job_pay.append(125)
